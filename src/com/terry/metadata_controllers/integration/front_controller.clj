...
(defn throw-no-handler-exception [request mappings]
  (let [uri (:uri request)
        http-method (:request-method request)]
    (throw (Exception. (str "Unable to match handler for " http-method " " uri " mappings: " mappings)))))
...
(defn handle
  ...
  (let [mappings (get-mappings namespaces cache?)
    ...
    (if matched-handler
      ...
      (throw-no-handler-exception request mappings))))
